...
let inkscapePromise = null;

function checkInkscape() {
  if (inkscapePromise === null) {
    inkscapePromise = _checkInkscape();
  }

  return inkscapePromise;
}

// check if the installed inkscape version is compatible (>= 1.0.0)
async function _checkInkscape() {
...
class Inkscape extends Stream {
  constructor(inkscapeArgs, { outputFormat = null, inputFormat = null } = {}) {
    super();
    ...
    // This value will be overwritten by applicable Inkscape CLI args.
    //
    ...
    // Set output format as applicable
    //
    if (setOutputFormat || outputFormat) {
      if (this.guiMode) {
        throw new Error(
    ...
    // Set input format as applicable
    //
    if (setInputFormat || inputFormat) {
